import React from 'react';
import { Truck, Construction, Radar, Package, ArrowRight, CheckCircle, Wrench, Users } from 'lucide-react';
import Header from '../components/Header';
import ContactSection from '../components/ContactSection';
import Footer from '../components/Footer';

const ServicesPage: React.FC = () => {
  const services = [
    {
      icon: Construction,
      title: 'Montážní plošina',
      description: 'Pronájem montážní plošiny Renault Maxity pro práce ve výškách až 16 metrů.',
      image: '/work/20241115_112046.jpg'
    },
    {
      icon: Wrench,
      title: 'Zemní práce',
      description: 'Výkopy, terénní úpravy a pokládka kabelů pomocí minibagru Kubota KX-019.',
      image: '/work/20250511_212135.jpg'
    },
    {
      icon: Truck,
      title: 'Autodoprava',
      description: 'Přeprava materiálu a poskytování kontejnerů pro stavební práce.',
      image: '/work/20220530_143714.jpg'
    },
    {
      icon: Radar,
      title: 'Montáž radarů',
      description: 'Instalace měřičů rychlosti a dopravních systémů.',
      image: '/work/20250511_211440.jpg'
    }
  ];

  const maxitySpecs = [
    { label: 'Maximální pracovní výška', value: '16 m' },
    { label: 'Nosnost košíku', value: '230 kg' },
    { label: 'Boční dosah', value: '9,5 m' },
    { label: 'Rozměry vozidla', value: '5,99 x 2,13 x 2,85 m' }
  ];

  const kubotaSpecs = [
    { label: 'Provozní hmotnost', value: '1 920 kg' },
    { label: 'Maximální hloubka kopání', value: '2 390 mm' },
    { label: 'Maximální dosah', value: '4 140 mm' },
    { label: 'Šířka podvozku', value: '990 mm' }
  ];

  const scrollToContact = () => {
    document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' });
  };

  return (
    <div className="min-h-screen bg-gray-50">
      <Header />
      
      {/* Hero Section */}
      <section className="pt-32 pb-20 bg-gradient-to-br from-citysound-green-50 to-citysound-blue-50">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="text-center mb-16">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 text-gray-900">
              Naše další <span className="text-citysound-green-600">služby a technika</span>
            </h1>
            <p className="text-xl text-gray-600 mb-8 leading-relaxed max-w-3xl mx-auto">
              Kromě veřejného osvětlení nabízíme komplexní služby včetně pronájmu specializované techniky 
              a provádění různých stavebních prací.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
            {services.map((service, index) => (
              <div key={index} className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
                <div className="relative">
                  <img
                    src={service.image}
                    alt={service.title}
                    className="w-full h-48 object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                  <div className="absolute top-4 left-4">
                    <div className="w-12 h-12 bg-citysound-green-600 rounded-full flex items-center justify-center">
                      <service.icon className="w-6 h-6 text-white" />
                    </div>
                  </div>
                </div>
                <div className="p-6">
                  <h3 className="text-xl font-bold text-gray-800 mb-3">{service.title}</h3>
                  <p className="text-gray-600 leading-relaxed">{service.description}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Services Details Section */}
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4">
              Detaily našich služeb
            </h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Každá z našich služeb je poskytována s maximální odborností a vlastní technikou.
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {/* Montážní plošina */}
            <div className="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer">
              <div className="relative h-48 overflow-hidden">
                <img
                  src="/work/20241115_112046.jpg"
                  alt="Montážní plošina Renault Maxity"
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                <div className="absolute top-4 left-4">
                  <div className="w-10 h-10 bg-citysound-green-600 rounded-full flex items-center justify-center">
                    <Construction className="w-5 h-5 text-white" />
                  </div>
                </div>
              </div>
              <div className="p-6">
                <h3 className="text-xl font-bold text-gray-800 mb-3">Montážní plošina Renault Maxity</h3>
                <p className="text-gray-600 text-sm leading-relaxed mb-4">
                  Profesionální práce ve výškách až 16 metrů. Ideální pro údržbu osvětlení a instalační práce.
                </p>
                <div className="space-y-2 text-sm text-gray-600">
                  <div className="flex justify-between">
                    <span>Max. výška:</span>
                    <span className="font-semibold text-citysound-green-600">16 m</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Nosnost:</span>
                    <span className="font-semibold text-citysound-green-600">230 kg</span>
                  </div>
                </div>
              </div>
            </div>

            {/* Minibagr */}
            <div className="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer">
              <div className="relative h-48 overflow-hidden">
                <img
                  src="/work/20250511_211440.jpg"
                  alt="Minibagr Kubota KX-019"
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                <div className="absolute top-4 left-4">
                  <div className="w-10 h-10 bg-citysound-green-600 rounded-full flex items-center justify-center">
                    <Wrench className="w-5 h-5 text-white" />
                  </div>
                </div>
              </div>
              <div className="p-6">
                <h3 className="text-xl font-bold text-gray-800 mb-3">Zemní práce Kubota KX-019</h3>
                <p className="text-gray-600 text-sm leading-relaxed mb-4">
                  Kompaktní minibagr pro výkopy, terénní úpravy a pokládku kabelového vedení.
                </p>
                <div className="space-y-2 text-sm text-gray-600">
                  <div className="flex justify-between">
                    <span>Hmotnost:</span>
                    <span className="font-semibold text-citysound-green-600">1 920 kg</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Max. hloubka:</span>
                    <span className="font-semibold text-citysound-green-600">2 390 mm</span>
                  </div>
                </div>
              </div>
            </div>

            {/* Autodoprava */}
            <div className="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer">
              <div className="relative h-48 overflow-hidden">
                <img
                  src="/work/20210107_150459.jpg"
                  alt="Autodoprava a kontejnery"
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                <div className="absolute top-4 left-4">
                  <div className="w-10 h-10 bg-citysound-green-600 rounded-full flex items-center justify-center">
                    <Truck className="w-5 h-5 text-white" />
                  </div>
                </div>
              </div>
              <div className="p-6">
                <h3 className="text-xl font-bold text-gray-800 mb-3">Autodoprava a kontejnery</h3>
                <p className="text-gray-600 text-sm leading-relaxed mb-4">
                  Přeprava materiálu, poskytování kontejnerů a odvoz stavebního odpadu.
                </p>
                <div className="space-y-1 text-sm text-gray-600">
                  <div className="flex items-center space-x-2">
                    <CheckCircle className="w-4 h-4 text-citysound-green-600" />
                    <span>Přeprava materiálu</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <CheckCircle className="w-4 h-4 text-citysound-green-600" />
                    <span>Kontejnery různých velikostí</span>
                  </div>
                </div>
              </div>
            </div>

            {/* Radary */}
            <div className="group bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer">
              <div className="relative h-48 overflow-hidden">
                <img
                  src="/work/20250511_205703.jpg"
                  alt="Instalace měřičů rychlosti"
                  className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                <div className="absolute top-4 left-4">
                  <div className="w-10 h-10 bg-citysound-green-600 rounded-full flex items-center justify-center">
                    <Radar className="w-5 h-5 text-white" />
                  </div>
                </div>
              </div>
              <div className="p-6">
                <h3 className="text-xl font-bold text-gray-800 mb-3">Montáž měřičů rychlosti</h3>
                <p className="text-gray-600 text-sm leading-relaxed mb-4">
                  Instalace moderních radarů a dopravních systémů pro zvýšení bezpečnosti.
                </p>
                <div className="space-y-1 text-sm text-gray-600">
                  <div className="flex items-center space-x-2">
                    <CheckCircle className="w-4 h-4 text-citysound-green-600" />
                    <span>Profesionální instalace</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <CheckCircle className="w-4 h-4 text-citysound-green-600" />
                    <span>Moderní technologie</span>
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="text-center mt-12">
            <button
              onClick={scrollToContact}
              className="bg-citysound-red-600 hover:bg-citysound-red-700 text-white px-8 py-4 rounded-lg font-semibold text-lg transition-all duration-300 transform hover:scale-105 flex items-center space-x-2 mx-auto"
            >
              <span>Nezávazně poptat služby</span>
              <ArrowRight className="w-5 h-5" />
            </button>
          </div>
        </div>
      </section>

      {/* Contact Section */}
      <ContactSection 
        customTitle="Máte zájem o některou z našich služeb?"
        customSubtitle="Kontaktujte nás pro nezávaznou konzultaci a cenovou nabídku."
      />
      
      <Footer />
    </div>
  );
};

export default ServicesPage;